...
app_tree=$1
app_commit=$2
source_path=contracts/deployment/v4
destination=$(cd "$(dirname "$0")" && pwd)/contracts/deployment/v4
...
if [ -e "$destination" ] || [ -L "$destination" ]; then
  [ -d "$destination" ] && [ ! -L "$destination" ] || { echo "vendored deployment/v4 must be a real directory" >&2; exit 1; }
  for frozen in README.md schema.json conformance.json; do
    [ -f "${destination}/${frozen}" ] && [ ! -L "${destination}/${frozen}" ] && cmp -s "${stage}/${frozen}" "${destination}/${frozen}" || {
      echo "vendored deployment/v4 grammar differs; a new contract version is required" >&2
      exit 1
    }
  done
  for entry in "$destination"/*; do
    [ -f "$entry" ] && [ ! -L "$entry" ] || { echo "vendored deployment/v4 contains a non-regular entry" >&2; exit 1; }
    case "$(basename "$entry")" in README.md|SHA256SUMS|schema.json|conformance.json|contract.json|UPSTREAM) ;; *) echo "vendored deployment/v4 contains an unmanaged entry" >&2; exit 1;; esac
  done
