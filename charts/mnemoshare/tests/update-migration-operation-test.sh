...
fixture=${test_dir}/contracts/migration-operation/v1
root=$(mktemp -d)
...
updater=${consumer}/charts/mnemoshare/tests/update-migration-operation.sh

"$updater" "$app" "$commit"
destination=${consumer}/charts/mnemoshare/tests/contracts/migration-operation/v1
...
printf '{}\n' > "${app}/contracts/migration-operation/v1/contract.json"
(cd "${app}/contracts/migration-operation/v1" && sha256sum README.md conformance.json contract.json schema.json > SHA256SUMS)
git -C "$app" add .
...
printf 'changed grammar\n' > "${app}/contracts/migration-operation/v1/schema.json"
(cd "${app}/contracts/migration-operation/v1" && sha256sum README.md conformance.json contract.json schema.json > SHA256SUMS)
git -C "$app" add .
