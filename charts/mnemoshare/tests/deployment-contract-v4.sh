...
expected_fingerprint=f42eb9987b37e8e02405c6ba3032eac5e73e853100e06e4e008c1a853c437aa6
...
render_profile() {
  ...
  render=$(helm template contract-v4 "$chart_dir" "${base[@]}" "$@")
...
expect_failure() {
	...
	shift
  if output=$(helm template bad-v4 "$chart_dir" "${base[@]}" "$@" 2>&1); then
