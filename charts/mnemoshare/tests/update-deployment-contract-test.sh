...
fixture=${test_dir}/contracts/deployment/v1
root=$(mktemp -d)
trap 'rm -rf "$root"' EXIT
app=${root}/app
consumer=${root}/consumer
mkdir -p "${app}/contracts/deployment/v1" "${consumer}/charts/mnemoshare/tests"
cp "$test_dir/update-deployment-contract.sh" "${consumer}/charts/mnemoshare/tests/"
cp "$fixture"/{README.md,SHA256SUMS,schema.json,conformance.json,contract.json} "${app}/contracts/deployment/v1/"
git -C "$app" init -q
git -C "$app" add .
...
"$updater" "$app" "$commit"
destination=${consumer}/charts/mnemoshare/tests/contracts/deployment/v1
...
printf '{}\n' > "${app}/contracts/deployment/v1/contract.json"
